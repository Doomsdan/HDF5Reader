...
#     max_speed_sec = hits_averages.argmax(axis=1)[1]  # sectornr max speed
    ...
    elif test[-2] / test[-1] > 0.75 and verbose == True:
        try:
    ...
    if (average_speed_maxsector / norm_uv_direct < 0.5 or
        average_speed_maxsector / norm_uv_direct > 2) and verbose == True:
...
#                    (date_time[dt_index_high] - date_time[dt_index_low]) / 2
            ...
            tmp_wind = method(time_block[0], time_block[1], sec, verbose, \
                              pd.to_datetime(return_time_act), wind=wind_)
...
#                    (date_time[dt_index_high] - date_time[dt_index_low]) / 2
    ...
    if format_ == 'string':
        return_time = pd.to_datetime(return_time)
